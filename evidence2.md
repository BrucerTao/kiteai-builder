...
SPEC02 的边界是**公网部署 + 公网真实付费 + 看板材料备齐**（主网 Track B 按用户决定跳过）：

| Phase | 内容 | 状态 | 证据 |
|---|---|---|---|
| 0 | 前期：fork `BrucerTao/kite-x402-services`、SSH 推送验证、平台决策（Render 免费档） | ✅ | §10 |
| 1 | 容器化：Dockerfile（交叉编译）+ .dockerignore + render.yaml + verify_public.sh | ✅ | §2、§3 |
| 2 | 部署：Render Web Service（Free，Docker runtime，Root Dir `services/frankfurter-fx`，5 env，`PAY_TO` Secret，健康检查 `/healthz`） | ✅ 部署 success | §11 |
| 3 | 外网复测：warm-up 后 `/healthz` 200、未付款 402 + 解码 4/4 PASS、TLS verify=0 | ✅ ALL_PASS | §11 |
| 4 | 公网真实付费调用：200 + ECB body + 结算 tx `0x186f8e20…`（链上实证收款方 == `PAY_TO`、0.001 pieUSD） | ✅（fxpayer2 路径，见 12.1-12.4） | §12 |
| 5 | 反向不结算：`base=NOTACURRENCY` → 404，无新结算、余额分毫不差 | ✅ | §12.5 |
| 6 | manifest 转正：`status: testnet` + `base_url`，`npm run validate` exit 0 | ✅（commit `98bbff1`） | §11 |
| 7 | 提交与看板：commit+push（`7a4304b`→`98bbff1`→`591749e`） | ✅ 推送完成；**看板网页提交待用户操作**（材料清单 §12.7） | §10、§12.7 |
| Track B | 主网真实结算（`status: live`） | ⏭️ 用户决定跳过（不花真钱） | §5 |

**一句话**：SPEC02 全部目标达成——公网 https 服务上线并通过全部协议断言，真实付费与反向不结算
均有链上实证，manifest 转正并推送；唯一剩余动作是用户在看板网页上提交（材料已备齐）。
过程中排查并记录了两个环境级问题（测试网链头滞后、SDK Flush 缺陷），见 §12.2/§12.6。
