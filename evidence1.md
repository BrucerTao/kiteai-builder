# Evidence 1 — SPEC01（`0917-SPEC01.md`）：本地 testnet 全链路 完成与验证

> 本文件汇总执行 `0917-SPEC01.md` 过程中**完成的事项与证明证据 / 终端输出文本**。
> 所有链上数据可在 Kite testnet 浏览器复核。日期：2026-09-17/18。
> SPEC02（公网部署轮）的完成情况见 `evidence2.md`。
...
## 0.1 SPEC01 各 Phase 完成状态

SPEC01 的边界是**本地全链路**（公网部署与看板推迟到 SPEC02）：

| Phase | 内容 | 状态 | 证据 |
|---|---|---|---|
| 0 | 前期准备：装 kpass、Go 环境、`PAY_TO` 钱包、faucet 领 100 pieUSD | ✅ | §2、E8（领水 tx `0xba0c9498…`） |
| 1 | 脚手架：拷 `templates/go-gin` → `services/frankfurter-fx`，`.env`，`go build`+`go vet` | ✅ | §3 目录清单 |
| 2 | 本地 402 验证：`/healthz` 200；未付款 402 头解码 8/8 断言 PASS | ✅ | E1、E2 |
| 3 | testnet 真实付费调用（自建付款方 fxpayer）：200 + body + 结算 tx，链上复核收款方 == `PAY_TO` | ✅ | E3、E8（tx `0xa56ccf24…`） |
| 4 | 反向验证：上游 4xx → 不结算、余额分毫未动 | ✅ | E9 |
| 5 | 主网交易逻辑验证（不花真钱）：切 `KITE_NETWORK=mainnet` 后 402 头 5/5 断言 PASS | ✅ | E5 |
| 6 | manifest + 文档 + `npm run validate` exit 0 | ✅ | E6 |
| — | kpass 打 localhost（SPEC01 原计划路径） | ❌ 不可行（后端服务端抓取）→ 改自建付款方 | E7 |

**一句话**：SPEC01 判据（验证清单 7 项）全部达成，唯一例外是 kpass 原生路径因
Passport 后端服务端抓取而无法打 localhost——以同 SDK 的自建付款方完成等价真实付费，
并由此论证了公网 https 部署的必要性（SPEC02 的动机）。
...
## 4. 遗留待办 → 全部已由 SPEC02 完成（见 `evidence2.md`）

| # | SPEC01 时点的待办 | SPEC02 结果 |
|---|---|---|
| 1 | `maintainer.github` 换成真实 GitHub owner | ✅ 已改 `BrucerTao`（commit `7a4304b`） |
| 2 | 公网 https 部署 | ✅ Render 免费档 `https://frankfurter-fx.onrender.com` |
| 3 | 外网复测 + `status: testnet` + `base_url` | ✅ verify_public.sh 4/4 PASS；commit `98bbff1` |
| 4 | 公网真实付费 + 反向不结算 | ✅ tx `0x186f8e20…`（fxpayer2 路径）+ 404 无结算；commit `591749e` |
| 5 | 还原 `package-lock.json` 无关改动 | ✅ 已还原，工作区干净（未提交任何无关改动） |

> 第 4 项说明：kpass 原生路径在 SPEC02 期间仍受阻（旧会话 401、prod 后端关闭 faucet、
> signup 验证码待用户输入），公网真实付费改用 **fxpayer2**（fxpayer 的宽签名窗口版）完成，
> x402 协议链路（402 → EIP-3009 签名 → verify → settle）与 kpass 完全一致，链上结算可复核。
